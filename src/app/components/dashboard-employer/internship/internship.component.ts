...
// Function to load internships after the companyId is set
  ...
  addInternship(){
    ...
    const internshipForm = {...this.internshipForm.value,companyId:this.companyId};
  ...
  updateInternship(id:string){
    ...
    this.internshipService.updateInternship(updateInternshipForm, id).subscribe({
      next: (response) => {
        ...
        this.data = this.data.map((internship: any) =>
          internship.id === id ? { ...internship, ...updateInternshipForm } : internship
        );      },
      error: (err) => {
  ...
  deleteInternship(id:string){
    this.internshipService.deleteInternship(id).subscribe({
      next: (response) => {
        this.data = this.data.filter((internship: { id: string; }) => internship.id !== id);
        this.isLoading = false;
  ...
  openModal2(id:any): void {
    ...
    if (modal) {
      this.updateInternshipForm.patchValue(this.data.find((internship: { id: string; }) => internship.id === id));
      modal.showModal();
    } else {
